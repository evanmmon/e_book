...
public class BookPage {
	...
	private int visibleHeight;//屏幕中可显示文本的高度
	...
	protected void init() {
		...
		lineCount = visibleHeight / lineHgight - 2;//屏幕的高除以字体的高-2
		isfirstPage = true;
		islastPage = false;
	...
	protected void slicePage() {
		...
		while (curPos < chapterLen) {
			Vector<String> lines = new Vector();//Vector是同步
			charBegin = curPos;
	...
	public boolean nextChapter() {
		order = chapter.getOrder();
		Chapter tempChapter = book.getChapter(order+1);
		if (tempChapter == null)
			return false;
		chapter = tempChapter;
		content = chapter.getContent();
	...
	public void draw(Canvas c) {
		...
		int titWidth = (int) paintBottom.measureText(chapter.getTitleId()+"  "+chapter.getTitleName());

		
		c.drawText(timeStr, marginWidth / 2, screenHeight - 5, paintBottom);
		c.drawText(chapter.getTitleId()+"  "+chapter.getTitleName(), screenWidth / 2 - titWidth / 2,
